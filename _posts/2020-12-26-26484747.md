...
```
说了这么多的废话, 好像和元类一点关系都没有. 其实不是的, 我们看下 `type` 的 `__new__` 方法的定义, 发现了吗? 第一个参数就是我们所说的元类. 需要说明的是, 如果使用 `type` 的 `__new__` 方法来创建类并指定元类, 那么就仅仅是指定了新创建类的元类, 新类并不是通过 `call` 元类来创建的.<br />
