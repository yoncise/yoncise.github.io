...
如果一个变量会被多个线程改写的话, 我们称之为 **共享变量**. 
如果一个变量只会被一个线程改写的话, 我们称之为 **私有变量**.

私有变量被多个线程读取是没有问题的, 而共享变量被多个线程读取时如果不加以限制就会导致问题.
我们会发现, 下面提到的互斥算法从本质上来讲都是 **通过使用私有变量来对共享变量的读取进行同步**.
...
这里的 `k` 是共享变量, Dijkstra 提出的算法利用 `b` 和 `c` 这两个私有变量来进行同步.
...
还有其他的互斥算法, 比如 Dekker 算法和面包店算法, 感兴趣的就自己查阅资料吧. 本质都差不多.

## 硬件方案

之所以会产生互斥问题就是因为对于共享变量的读取与修改不是原子操作,
如果在硬件上保证了 **读取与修改是原子操作**, 也就可以有效的对临界区进行管理.
...
硬件提供的 TS 指令可以看做是一个函数, 且这个函数的执行是个原子操作:
