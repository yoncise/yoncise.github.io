...
我们先考虑 DFA 的第一列 (状态 0), 只有当输入字符合 Pat 的第一个字符相同, DFA 才会进入 (1) 状态 (即匹配了一个字符),
...
总结下就是, 当在某个状态不匹配时, DFA 会进入 (0) 状态 (右移一位), 这时我们可以根据已经匹配的字符来来推断, 
当匹配到当前字符时 DFA 会处于哪个状态, 那么当前状态的跳转情况就和这个状态一样. 本质也是一种递归的思想,
...
算法里的 x 是比较重要的, 按照之前的说法我们是需要每次都回溯的, 但是因为 Pat 的字符串是不变的, 
那么我们只需要用 x 来记录上个状态在在不匹配时等同于的状态就行, 每个状态处理的最后更新下 x 就行.

这个就是 Knuth DFA 版的 KMP 算法, 我觉得这个版本是比较好理解的. 但这个版本有一个缺点就是, 
需要使用 MR (Pat 的长度乘以可输入字符的长度) 的额外空间. Pratt 在 Knuth 的 DFA 版本上改进了下, 只需要
额外使用 M 的空间 (即和可输入字符长度无关, 当然效率会低点), 就有了现在通常所有的 KMP 算法. 
