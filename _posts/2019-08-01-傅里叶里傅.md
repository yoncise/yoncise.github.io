...
因为 $$ T \to \infty $$, 原先 $$ c_n $$
中的系数 $$ \frac{1}{T} $$ 变成 $$ d\delta $$ "挪" 到了 `f(x)` 中, 原先 `f(x)` 中的累加符号换成了积分.
...
$$
\begin{align*}
f[k] &= \int_{\infty} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta \\

c_{\delta} &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{\frac{-2\pik}{N}\delta i} dx\right)
\end{align*}
$$
...
就需要对于每个 `k`, 我们有 $$ e^{\frac{-2\pik}{N}\delta i} = 1 $$, 显然只要 $$ \Delta \frac{k}{N} $$ 为非零整数即可.
...
$$
\begin{align*}
f[k] &= dx \int_{\infty} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta \\ 

c_{\delta} &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{\frac{-2\pik}{N}\delta i} \right)
\end{align*}
$$

现在还有个问题, 前面分析过了, $$  c_{\delta} $$ 是周期函数, 所以显然 $$ c_{\delta} e^{\frac{2\pik}{N}\delta i} $$
...
$$
\begin{align*}
\int_{\infty} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta &= f[k] \int_{\infty}  \left( e^{\frac{2\pik}{N}\delta i} e^{\frac{-2\pik}{N}\delta i} \right) \\

\int_{\infty} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta &= f[k] \int_{\infty}(1)
\end{align*}
$$

再利用 $$ c_{\delta} e^{\frac{2\pik}{N}\delta i} $$ 的周期为 $$ N $$, 最终, 在 $$ \infty $$ 的积分就变成:

$$
\begin{align*}
\int_{N} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta &= f[k] N \\

\frac{1}{N}\int_{N} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta &= f[k]
\end{align*}
$$
...
$$
\begin{align*}
f[k] &= \frac{1}{N}\int_{N} \left(c_{\delta} e^{\frac{2\pik}{N}\delta i}\right) d\delta \\
...
$$
\begin{align*}
f[k] &= s\sum_{n \in \frac{1}{s}} \left(c_n e^{\frac{2 \pi n}{T} \frac{k}{N} i}\right) d\delta \\

c_n &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{\frac{-2 \pi n}{T} \frac{k}{N} i} \right)
\end{align*}
$$

同样的道理, 利用 `f[k]` 周期为 `T` 的性质, 同时将 $$ d\delta $$ "挪" 到 $$ c_n $$ 中:

$$
c_n = d\delta \sum_{k=-\infty}^{+\infty} \left(f[k] e^{\frac{-2 \pi n}{T} \frac{k}{N} i} \right) \\

c_n = \frac{1}{T} \sum_{k \in T} \left(f[k] e^{\frac{-2 \pi n}{T} \frac{k}{N} i} \right)
$$

假设一个 `T` 周期内的采样点数是 `N`, 那么 `T=Ns`, 最终离散傅里叶变换的结果为:

$$
f[k] = \frac{1}{N}\sum_{n \in N} \left(c_n e^{\frac{2 \pi n}{N} k i}\right) \\
...
在时域非周期, 在频域就是连续. 整理后, 我们可以将最开始的表格扩充下:
