...
怎么处理呢? 想像一下, 所谓的连续函数不过就是采样间隔无穷小的函数, 令采样间隔为 `s`:

$$
\begin{align*}
f[k] &= \int_{\infty} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta \\

c_{\delta} &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{-2\pi\delta ks i} dx\right)
\end{align*}
$$
...
$$
\begin{align*}
f[k] &= dx \int_{\infty} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta \\ 

c_{\delta} &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{-2\pi\delta ks i} \right)
\end{align*}
$$

现在还有个问题, 前面分析过了, $$  c_{\delta} $$ 是周期函数, 所以显然 $$ c_{\delta} e^{2\pi\delta ks i} $$ 也是个周期函数, 
对一个周期函数, 在 $$ \infty $$ 上积分, 显然值只可能是: 无穷大, 无穷小或者零. 这种情况下,
再乘以一个无穷小的 `dx`, 最终的值究竟是什么呢? 我们反过来看, $$ c_{\delta} $$ 是个连续周期函数,
那么我们是不是也可以对它进行傅里叶级数分解呢? 所以:

$$
\begin{align*}
\int_{\infty} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta &= f[k] \int_{\infty}  \left( e^{2\pi\delta ks i} e^{-2\pi\delta ks i} \right) \\

\int_{\infty} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta &= f[k] \int_{\infty}(1)
\end{align*}
$$
...
$$
\begin{align*}
\int_{\frac{1}{s}} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta &= f[k] \frac{1}{s} \\

s\int_{\frac{1}{s}} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta &= f[k]
\end{align*}
$$
...
$$
\begin{align*}
f[k] &= s\int_{\frac{1}{s}} \left(c_{\delta} e^{2\pi\delta ks i}\right) d\delta \\
...
$$
\begin{align*}
f[k] &= s\sum_{n \in \frac{1}{s}} \left(c_n e^{\frac{2 \pi n}{T} ks i}\right) d\delta \\

c_n &= \sum_{k=-\infty}^{+\infty} \left(f[k] e^{\frac{-2 \pi n}{T} ks i} \right)
\begin{align*}
$$

同样的道理, 利用 `f[k]` 周期为 `T` 的性质, 同时将 $$ d\delta $$ 挪到 $$ c_n $$ 中:
...
$$
f[k] = s\sum_{n \in \frac{1}{s}} \left(c_n e^{\frac{2 \pi n}{N} k i}\right)
$$

$$
c_n = \frac{1}{Ns} \sum_{k \in Ns} \left(f[k] e^{\frac{-2 \pi n}{N} k i} \right)
$$
