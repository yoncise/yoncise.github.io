...
- `T2` 可以马上 S 锁, 最终 `T1` 和 `T2` 在 `r` 上各有一个 S 锁
- `T2` 无法立即获得一个 X 锁

如果 `T1` 在 `r` 上有一个排它锁 (X), 那么 `T2` 在 `r` 无论是 S 锁还是 X 锁都无法立即获得.
...
那么 InnoDB 就会在所有行上加上锁, 等于将整个表锁上了.

### 意向锁

InnoDB *表级锁* 是通过意向锁实现的, 意向锁有两种: Intention shared (IS), Intention exclusive (IX).
...
最后修改的时候判断下数据的时间戳和之前读取的时间戳一致不一致 (这里的时间戳充当了锁的角色),
